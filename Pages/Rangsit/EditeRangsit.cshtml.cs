using Inventory2.Pages.Rangsit;
...
{
    public class EditeRangsitModel : PageModel
    {
        ...
        public void OnGet()
        {
            ...
            try
            {
                String conectionString = "Server=tcp:inventory1805.database.windows.net,1433;Initial Catalog=inventorysystem;Persist Security Info=False;User ID=benyatip.phad;Password=Baifern_1805;MultipleActiveResultSets=False;Encrypt=True;TrustServerCertificate=False;Connection Timeout=30;";
            ...
            try
            {
                ...
                using (SqlConnection connection = new SqlConnection(conectionString))
                {
                    ...
                    String sql = "UPDATE stocks " +
                        ...
                        "WHERE itemid=@itemid"; // เพิ่ม @itemid ในคำสั่ง SQL
                    using (SqlCommand command = new SqlCommand(sql, connection))
                    {
                        ...
                        command.Parameters.AddWithValue("@itemid", stockInfo.itemid); // เพิ่มพารามิเตอร์ @itemid
